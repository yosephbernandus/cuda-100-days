...
void saveGrayscaleBMP(const char* filename, unsigned char* data, int width, int height) {
    ...
    int fileSize = 54 + 1024 + imageSize; // 54 header + 1024 color table + image data
...
int main(int argc, char** argv) {
    ...
    // Launch the kernel
    colortoGrayscaleConvertion<<<gridSize, blockSize>>>(d_output, d_input, width, height);
